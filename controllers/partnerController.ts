...
export class PartnerController {
    ...
    public createPartner(req: Request, res: Response) {
        ...
        if (req.body.name && req.body.name.first_name && req.body.name.middle_name && req.body.name.last_name &&
            ...
            this.partnerService.createUser(user_params, (err: any, user_data: IPartner) => {
                if (err) {
                    mongoError(err, res);
                } else {
    ...
    public getPartner(req: Request, res: Response) {
        if (req.params.id) {
            ...
            this.partnerService.filterUser(user_filter, (err: any, user_data: IPartner) => {
                if (err) {
                    mongoError(err, res);
                } else {
        ...
        if (req.params.id &&
            ...
            this.partnerService.filterUser(user_filter, (err: any, user_data: IPartner) => {
                if (err) {
                    ...
                    this.partnerService.updateUser(user_params, (err: any) => {
                        if (err) {
                            mongoError(err, res);
                        } else {
    ...
    public deletePartner(req: Request, res: Response) {
        if (req.params.id) {
            this.partnerService.deleteUser(req.params.id, (err: any, delete_details) => {
                if (err) {
                    mongoError(err, res);
